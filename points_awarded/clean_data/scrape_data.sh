#########################################################################
#  Scrape the .txt file for the points data and write to .dat files.    #
...
for year in 07 08 09 10 11 12 13 14 15 16; do
    ...
    for p in `seq 590 -10 100`; do
        num=`cat  $file | tail -60 | grep -v "from" | grep "$p\/" | grep "$p" | awk '{print $6}' | sed 's/,//g'`
        echo $p $num >> points.20$year.dat
